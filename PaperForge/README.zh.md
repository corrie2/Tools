...
> Windows 用户注意：将 `~/my-vault` 替换为实际路径，如 `C:\Users\你的用户名\my-vault`
...
```cmd
:: Windows CMD
set MOONSHOT_API_KEY=sk-xxxxxxxx
...
> Ollama 不需要真正的 API Key，设置一个占位值 `"ollama"` 即可。
...
```cmd
:: Windows CMD
set OPENROUTER_API_KEY=sk-xxxxxxxx
...
PaperForge 可以自动检测环境变量中的 API Key，并引导你完成配置：
